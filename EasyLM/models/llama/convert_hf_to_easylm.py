...
def main(args):
    ...
    jax_weights = {
        'transformer': {
            ...
            'h': {
                '%d' % (layer): {
                    'attention': {
                        'wq': {'kernel': inverse_permute(ckpt[f'layers.{layer}.self_attn.q_proj.weight'].numpy()).transpose()},
                        'wk': {'kernel': inverse_permute(ckpt[f'layers.{layer}.self_attn.k_proj.weight'].numpy()).transpose()},
                        'wv': {'kernel': ckpt[f'layers.{layer}.self_attn.v_proj.weight'].numpy().transpose()},
    ...
    if args.streaming:
        StreamingCheckpointer.save_train_state_to_file(
            jax_weights, args.output_file
        )
    else:
        with mlxu.open_file(args.output_file, 'wb') as fout:
            fout.write(flax.serialization.msgpack_serialize(jax_weights, in_place=True))
                
    print(f'Save finished!!! take time: {time.time() - start} save path: {args.output_file}')

if __name__ == '__main__':

    parser = argparse.ArgumentParser(description='hf to easylm format script')

    parser.add_argument('--checkpoint_dir', type=str, help='Need to be converted model weight dir. it is a dir')
    parser.add_argument('--output_file', type=str,  help='Save model weight file path, it is a file.')
